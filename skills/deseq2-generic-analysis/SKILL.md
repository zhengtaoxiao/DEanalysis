...
3. Run `deseq2_generic.R` with selected options.
4. Summarize key outputs for downstream steps.
5. When requested, emit interface-aligned JSON using schema templates in `templates/`.
...
- For strict input/output contracts, read:
  - `../../templates/skill_input.schema.json`
  - `../../templates/skill_output.schema.json`
